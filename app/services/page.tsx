        ...
        <section className="bg-gray-50 py-16 lg:py-32">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="relative">
              {/* Ligne verticale - cachée sur mobile, visible sur desktop */}
              <div className="hidden lg:block absolute left-8 top-0 bottom-0 w-1 bg-gradient-to-b from-[#7384bc] via-[#fd733f] via-green-500 to-purple-600 rounded-full"></div>
              
              <div className="space-y-12 lg:space-y-20">
                {services.map((service, index) => (
                  <div key={index} className="relative">
                    {/* Numéro sur la ligne - adapté pour mobile */}
                    <div className="lg:absolute lg:left-0 w-12 h-12 lg:w-16 lg:h-16 bg-white rounded-full border-4 border-gray-50 flex items-center justify-center shadow-lg lg:z-10 mx-auto lg:mx-0 mb-6 lg:mb-0">
                      <span className={`text-lg lg:text-2xl font-bold ${service.color} font-architecture`}>
                        {service.number}
                      </span>
                    </div>
                    
                    {/* Contenu */}
                    <div className="lg:ml-24 bg-white rounded-2xl lg:rounded-3xl p-6 sm:p-8 lg:p-12 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 lg:hover:-translate-y-2 w-full">
                      <div className="grid lg:grid-cols-2 gap-6 lg:gap-12 items-start">
                        <div className="space-y-4 lg:space-y-6">
                          <div className={`w-16 h-16 lg:w-20 lg:h-20 ${service.bgColor} rounded-2xl flex items-center justify-center mb-4 lg:mb-6 mx-auto lg:mx-0`}>
                            <service.icon className={`w-8 h-8 lg:w-10 lg:h-10 ${service.color}`} />
                          </div>
                          
                          <h3 className="text-2xl lg:text-3xl font-bold text-slate-900 font-architecture text-center lg:text-left">
                            {service.title}
                          </h3>
                          
                          <p className="text-base lg:text-lg text-slate-600 leading-relaxed font-architecture text-center lg:text-left">
                            {service.description}
                          </p>
                          
                          <div className="flex justify-center lg:justify-start">
                            <a href="/contact" className={`inline-flex items-center px-6 py-3 bg-gradient-to-r from-slate-900 to-slate-700 text-white rounded-xl hover:from-slate-800 hover:to-slate-600 transition-all duration-300 font-architecture group text-sm lg:text-base`}>
                        ...
                        <div className="space-y-3 lg:space-y-4">
                          <h4 className="text-lg lg:text-xl font-bold text-slate-900 font-architecture mb-4 lg:mb-6 text-center lg:text-left">
                            Ce qui est inclus :
                          </h4>
                          {service.features.map((feature, idx) => (
                            <div key={idx} className="flex items-start space-x-3 lg:space-x-4 p-3 lg:p-4 bg-gray-50 rounded-xl">
                              <CheckCircle className={`w-5 h-5 lg:w-6 lg:h-6 ${service.color} flex-shrink-0 mt-0.5`} />
                              <span className="text-sm lg:text-base text-slate-700 font-architecture font-medium">{feature}</span>
                            </div>
                          ))}
                        </div>
                      </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
              {[
                ...
                <div key={index} className="relative group">
                  <div className="bg-white rounded-2xl p-6 lg:p-8 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
                    <div className={`w-14 h-14 lg:w-16 lg:h-16 ${process.color} rounded-2xl flex items-center justify-center mx-auto mb-4 lg:mb-6 group-hover:scale-110 transition-transform duration-300`}>
                    ...
                    <div className="text-center">
                      ...
                      <h3 className="text-lg lg:text-xl font-bold text-slate-900 mb-3 lg:mb-4 font-architecture">
                        {process.title}
                      </h3>
                  ...
                  {/* Flèche de connexion - seulement sur desktop */}
                  {index < 3 && (
