      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center">
            <div className="flex items-center space-x-4">
              <div className="relative">
                <div className="w-12 h-12 bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 rounded-xl flex items-center justify-center shadow-lg transform rotate-3 hover:rotate-0 transition-transform duration-300">
                  <span className="text-white font-bold text-xl tracking-tight">O</span>
                </div>
                <div className="absolute -top-1 -right-1 w-6 h-6 bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg flex items-center justify-center shadow-md transform -rotate-12">
                  <span className="text-white font-bold text-xs">D</span>
                </div>
              </div>
              <div className="flex flex-col">
                <span className="text-2xl font-bold text-slate-900 leading-tight tracking-tight font-architecture">
                  Olivier <span className="text-[#fd733f]">DANG</span>
                </span>
                <span className="text-sm text-slate-500 font-medium font-architecture">Expert Immobilier & Travaux</span>
              </div>
            </div>
          </div>
        ...
        <div className={`lg:hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
          <div className="py-4 space-y-4 border-t border-slate-100">
            <Link href="/" className="block text-slate-700 hover:text-[#fd733f] font-semibold py-2 transition-colors duration-200 font-architecture">
              Accueil
            </Link>
            <Link href="/a-propos" className="block text-slate-700 hover:text-[#fd733f] font-semibold py-2 transition-colors duration-200 font-architecture">
              À propos
            </Link>
            <Link href="/services" className="block text-slate-700 hover:text-[#fd733f] font-semibold py-2 transition-colors duration-200 font-architecture">
              Services
            </Link>
            <Link href="/contact" className="block text-slate-700 hover:text-[#fd733f] font-semibold py-2 transition-colors duration-200 font-architecture">
              Contact
            </Link>
          </div>
        </div>
